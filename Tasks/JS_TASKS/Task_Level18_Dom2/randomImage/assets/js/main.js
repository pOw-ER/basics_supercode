...
let hText = document.createTextNode('Zufälliges Bild mit JavaScript anzeigen')
...
onclick.value = "showRandomImage(event)"
...
images.style.margin = "10px auto"
function getRandomInt(max) {
  return Math.floor(Math.random() * Math.floor(max));
}

let showRandomImage = (event) => {
  content.appendChild(images)
  images.src = data[getRandomInt(data.length)].download_url;

  event.preventDefault();
}

// wahrscheinlich ist Url falsh
